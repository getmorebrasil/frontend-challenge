...
async function getShowCaseMovie(): Promise<IResponse<IShowCaseIMovie>> {
  try {
    const response = await baseService.client.get<ITheMovieDBListResponse<IShowCaseIMovie[]>>(URLS.TOP_RATED);
...
const movieService = { getPopulars, getShowCaseMovie, getMovies, getMovieCredits }
